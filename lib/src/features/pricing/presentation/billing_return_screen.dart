...
class BillingReturnScreen extends ConsumerWidget {
  const BillingReturnScreen({required this.flow, this.status, super.key});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (!kIsWeb && defaultTargetPlatform == TargetPlatform.iOS) {
      return Scaffold(
        appBar: AppBar(title: const Text('App Store Billing')),
        body: Center(
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Subscriptions in this iOS app are managed through the App Store. Open pricing to subscribe, restore, or manage your plan.',
                    textAlign: TextAlign.center,
                    style: TextStyle(
                      fontSize: 15,
                      height: 1.45,
    ...
    if (isCheckout && isSuccess && !isPro) {
      ...
      leading = Icon(Icons.info_outline_rounded, size: 32, color: cs.primary);
      ...
      leading = Icon(Icons.open_in_new_rounded, size: 32, color: cs.primary);
    }
              ...
              children: [
                ...
                FilledButton(
                  onPressed: () => context.go(
                    isCheckout && isSuccess ? '/app/home' : '/app/pricing',
                  ),
                  child: Text(
                    isCheckout && isSuccess
                        ? 'Go to dashboard'
                        : (isCheckout ? 'Open pricing' : 'Back to app'),
                  ),
                ),
                const SizedBox(height: 10),
                TextButton(
