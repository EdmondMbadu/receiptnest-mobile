...
class AiRepository {
  ...
  Stream<List<AiChatHistoryItem>> watchHistory(String userId, {int limit = 20}) {
    return _db
        .collection('users')
        .doc(userId)
        .collection('aiChats')
        ...
        .limit(limit)
        .snapshots()
        .map((snapshot) => snapshot.docs.map(AiChatHistoryItem.fromSnapshot).toList());
  }

  Future<List<AiChatMessage>> loadChat(String userId, String chatId) async {
    final doc = await _db.collection('users').doc(userId).collection('aiChats').doc(chatId).get();
  ...
  Future<String> ensureChat(String userId, {String? preferredId}) async {
    ...
    final ref = await _db.collection('users').doc(userId).collection('aiChats').add({
      'title': 'New chat',
      'messages': const [],
      'messageCount': 0,
  ...
  Future<void> deleteChat(String userId, String chatId) {
    return _db.collection('users').doc(userId).collection('aiChats').doc(chatId).delete();
  }
  ...
  Future<List<AiChatMessage>> sendMessage({
    ...
    final answer = result is Map ? (result['response']?.toString() ?? 'No response received.') : 'No response received.';
  ...
  Future<void> persistChat(String userId, String chatId, List<AiChatMessage> messages) async {
    final title = _buildTitle(messages);
    await _db.collection('users').doc(userId).collection('aiChats').doc(chatId).set({
      'title': title,
  ...
  Map<String, dynamic> buildInsightData({
    ...
    final totalSpend = selectedReceipts.fold<double>(0, (runningTotal, r) => runningTotal + (r.totalAmount ?? 0));

    final byCategory = <String, double>{};
    for (final receipt in selectedReceipts) {
      final key = receipt.category?.name ?? 'Other';
      byCategory[key] = (byCategory[key] ?? 0) + (receipt.totalAmount ?? 0);
    }
    ...
    final topCategories = byCategory.entries
        .map((entry) => {
              'name': entry.key,
              'total': entry.value,
              'percentage': totalSpend > 0 ? ((entry.value / totalSpend) * 100).round() : 0,
            })
        .toList()
    ...
    final dailyAverage = daysWithSpending == 0 ? 0 : totalSpend / daysWithSpending;
    ...
    final prevSpend = receipts.where((receipt) {
      final date = receipt.effectiveDate;
      if (date == null) return false;
    ...
    for (final receipt in receipts) {
      ...
      final merchant = receipt.merchant?.canonicalName ?? receipt.merchant?.rawName ?? 'Unknown';
      allByCategory[cat] = (allByCategory[cat] ?? 0) + amount;
      allByMerchant[merchant] = (allByMerchant[merchant] ?? 0) + amount;
    }

    final allTopCategories = allByCategory.entries
        .map((entry) => {
              'name': entry.key,
              'total': entry.value,
              'percentage': allSpend > 0 ? ((entry.value / allSpend) * 100).round() : 0,
            })
        .toList()
      ..sort((a, b) => (b['total'] as double).compareTo(a['total'] as double));

    final allTopMerchants = allByMerchant.entries
        .map((entry) => {
              'name': entry.key,
              'total': entry.value,
              'percentage': allSpend > 0 ? ((entry.value / allSpend) * 100).round() : 0,
    ...
    return {
      ...
      'highestSpendDay': highestDay == 0 ? null : {'day': highestDay, 'amount': highestAmount},
      ...
      'allTime': {
        ...
        'firstMonth': monthlySummariesData.isEmpty ? null : monthlySummariesData.first['monthId'],
        'lastMonth': monthlySummariesData.isEmpty ? null : monthlySummariesData.last['monthId'],
  ...
  String _buildTitle(List<AiChatMessage> messages) {
    final firstUser = messages.where((m) => m.role == 'user' && m.content.trim().isNotEmpty).firstOrNull;
    if (firstUser == null) return 'New chat';

    final normalized = firstUser.content.trim().replaceAll(RegExp(r'\s+'), ' ');
    return normalized.length <= 56 ? normalized : '${normalized.substring(0, 56)}...';
  }
}
